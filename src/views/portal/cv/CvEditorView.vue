...
// Form fields
const name = ref(portal.user.value?.name ?? '')
const trade = ref(portal.user.value?.trade ?? '')
const specialty = ref(portal.user.value?.specialty ?? '')
const location = ref(portal.user.value?.location ?? '')
...
const skills = ref<string[]>(['Kardex', 'Control de materiales', 'Tukuy Obra', 'Excel operativo'])
const newSkill = ref('')
...
function addExperience() {
  experiences.value.push({
    ...
    status: 'Declarado',
    summary: 'Escribe aquí los logros, responsabilidades y tareas realizadas durante el proyecto.'
  })
}
...
function saveCv() {
  ...
  showNotification('¡CV guardado con éxito! Los datos fueron actualizados en tu perfil.')
}

function goBack() {
  router.push('/tukuy-academy/cv')
}
...
<template>
  <PortalSection v-if="portal.user.value" wide :centered="false">
    ...
    <Transition name="fade">
      <div
        v-if="savedMessage"
        class="mb-6 flex items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-800"
      >
    ...
    <div class="grid gap-6 lg:grid-cols-2">
      ...
      <div class="grid gap-6">
        <Card class="border-slate-200 shadow-sm">
          ...
          <CardContent class="grid gap-4">
            <div class="grid gap-4 sm:grid-cols-2">
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase text-muted-foreground">Nombre completo</label>
                <Input v-model="name" type="text" placeholder="Ej. Carlos Quispe" class="h-11" />
              </div>
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase text-muted-foreground">Fecha de nacimiento</label>
            ...
            <div class="grid gap-4 sm:grid-cols-3">
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase text-muted-foreground">Oficio principal</label>
                <Input v-model="trade" type="text" placeholder="Ej. Auxiliar de almacén" class="h-11" />
              </div>
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase text-muted-foreground">Especialidad</label>
                <Input v-model="specialty" type="text" placeholder="Ej. Almacén y logística" class="h-11" />
              </div>
              <div>
                <label class="mb-1.5 block text-xs font-semibold uppercase text-muted-foreground">Ubicación</label>
                <Input v-model="location" type="text" placeholder="Ej. Lima, Ate" class="h-11" />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card class="border-slate-200 shadow-sm">
          <CardHeader class="flex flex-row items-center justify-between">
            ...
            <Button size="sm" variant="outline" class="gap-1" @click="addExperience">
              <Plus class="h-4 w-4" />
              Agregar
            </Button>
          </CardHeader>
          <CardContent class="grid gap-5">
            ...
            >
              <button
                type="button"
                class="absolute right-3 top-3 text-slate-400 hover:text-red-500 transition-colors"
              ...
              <div class="grid gap-4 sm:grid-cols-2">
                <div>
                  <label class="mb-1 block text-xs font-semibold text-muted-foreground">Puesto / Cargo</label>
                  <Input v-model="exp.role" type="text" class="h-9" />
                </div>
                <div>
                  <label class="mb-1 block text-xs font-semibold text-muted-foreground">Proyecto / Empresa</label>
              ...
              <div class="mt-3 grid gap-4 sm:grid-cols-3">
                <div>
                  <label class="mb-1 block text-xs font-semibold text-muted-foreground">Periodo</label>
                  <Input v-model="exp.period" type="text" class="h-9" placeholder="Ene 2025 - Oct 2025" />
                </div>
                <div>
                  <label class="mb-1 block text-xs font-semibold text-muted-foreground">Ubicación</label>
                  <Input v-model="exp.location" type="text" class="h-9" />
                </div>
                <div>
                  <label class="mb-1 block text-xs font-semibold text-muted-foreground">Fuente</label>
                  <Input v-model="exp.source" type="text" disabled class="h-9 bg-slate-100/80" />
                </div>
              </div>

              <div class="mt-3">
                <label class="mb-1 block text-xs font-semibold text-muted-foreground">Logros y funciones</label>
                <textarea
                  v-model="exp.summary"
                  rows="2"
          ...
          <CardContent class="grid gap-4">
            ...
            <div class="flex flex-wrap gap-2">
              <Badge
                ...
                :key="skill"
                variant="secondary"
                class="gap-1.5 px-3 py-1 text-sm bg-blue-50 text-[#0B3A78]"
              >
                {{ skill }}
                <button type="button" class="text-red-500 hover:text-red-700" @click="removeSkill(idx)">×</button>
              </Badge>
            </div>
          </CardContent>
        </Card>
      ...
      <div class="sticky top-6">
        <Card class="border-slate-200 shadow-lg overflow-hidden bg-white">
          <div class="bg-[#0B3A78] p-6 text-white">
            <div class="flex justify-between items-start">
              <div>
                <p class="text-xs uppercase tracking-wider text-blue-200">Currículum Vitae Inteligente</p>
                <h2 class="mt-1 text-2xl font-bold tracking-tight">{{ name || 'Tu Nombre Completo' }}</h2>
                <p class="text-sm text-blue-100/90 font-medium mt-1">{{ trade || 'Tu Oficio' }}</p>
              </div>
            ...
            <div class="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-xs text-blue-100">
              <span v-if="location">📍 {{ location }}</span>
              <span v-if="birthDate">📅 Nacimiento: {{ previewBirthDate(birthDate) }}</span>
            </div>
          </div>
          ...
          <CardContent class="p-6 grid gap-6">
            ...
            <div class="grid gap-1.5">
              <h3 class="text-xs font-bold uppercase text-[#0B3A78] tracking-wider border-b border-blue-100 pb-1">
                Resumen Profesional
              </h3>
              <p class="text-sm leading-relaxed text-slate-600">
                {{ summary || 'Describe tu perfil profesional en el editor de la izquierda.' }}
              </p>
            </div>
            ...
            <div class="grid gap-3">
              ...
              <div class="grid gap-4">
                <div v-if="experiences.length === 0" class="text-sm text-slate-400 italic">
                  No hay experiencia agregada todavía.
                </div>
                <div
                  v-for="exp in experiences"
                  :key="exp.id"
                  class="relative pl-4 border-l-2 border-slate-200"
                >
                  <div class="flex justify-between items-start gap-2">
                    <strong class="text-sm text-slate-900 block font-bold">{{ exp.role }}</strong>
                    <span class="text-[11px] font-semibold text-slate-500 shrink-0">{{ exp.period }}</span>
                  </div>
                  <div class="flex items-center gap-2 mt-0.5">
                    <span class="text-xs text-slate-600 font-medium">{{ exp.project }}</span>
                    <span class="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.2 rounded">{{ exp.source }}</span>
                  </div>
                  <p class="text-xs text-slate-500 mt-2 leading-relaxed">{{ exp.summary }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="grid gap-2">
              ...
              <div class="flex flex-wrap gap-1.5 mt-1">
                <span
                  ...
                  :key="skill"
                  class="text-xs bg-slate-100 text-slate-700 px-2.5 py-1 rounded-full font-medium"
                >
                  ✓ {{ skill }}
                </span>
              </div>
          ...
          <div class="bg-slate-50 border-t border-slate-100 px-6 py-4 flex justify-between items-center">
            <span class="text-[10px] text-muted-foreground">Generado por Tukuy Academy & Jobs</span>
            <Button size="sm" variant="ghost" class="gap-1 text-xs text-[#0B3A78]" disabled>
