...
function handleConfirmApply() {
  ...
  const t3 = setTimeout(() => {
    if (selectedJob.value) {
      ...
      localStorage.setItem('tukuy_applied_jobs', JSON.stringify(appliedJobIds.value))
  ...
  <PortalSection wide :centered="false">
    ...
    <section v-if="portal.scopeFilter.value === 'all' && portal.forYouJobs.value.length" class="grid gap-4">
      <div class="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p class="text-xs font-bold uppercase text-[#0B3A78]">Hecho para ti</p>
          <h2 class="text-xl font-bold text-[#07152B]">Vacantes alineadas con tu perfil</h2>
    ...
    <section class="grid gap-4">
      <div class="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 class="text-xl font-bold text-[#07152B]">
    ...
    >
      <div 
        class="relative w-full max-w-md overflow-hidden rounded-xl border border-white/20 bg-white shadow-2xl transition-all duration-300"
      >
        <!-- Modal Header -->
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
          <div class="flex items-center gap-2">
            <Briefcase class="h-5 w-5 text-[#0B3A78]" />
            <h3 class="text-sm font-extrabold text-[#0B3A78] uppercase tracking-wide">
              Postulación Laboral
            </h3>
          </div>
          <button 
        ...
        <template v-if="applicationStep === 'details'">
          <div class="p-6 space-y-5 max-h-[70vh] overflow-y-auto">
            <!-- Job title summary -->
            <div class="rounded-lg bg-slate-50 p-4 border border-slate-150">
              <h4 class="text-base font-bold text-slate-900 leading-snug">{{ selectedJob.title }}</h4>
              <p class="mt-1 text-xs text-slate-500 flex flex-wrap gap-x-3">
                <span class="font-semibold text-slate-700">{{ selectedJob.company }}</span>
                <span>·</span>
                <span>{{ selectedJob.location }}</span>
              </p>
              <div class="mt-2.5 flex items-center gap-1.5 text-[11px] text-amber-700 bg-amber-50 border border-amber-200/50 px-2.5 py-1 rounded-md w-fit font-medium">
            ...
            <!-- Skills match matrix -->
            <div class="space-y-2">
              <h5 class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Compatibilidad de Requisitos</h5>
              <div class="grid gap-2 sm:grid-cols-2">
                <div 
                  v-for="tag in selectedJob.tags" 
                  :key="tag"
                  class="flex items-center gap-2 rounded-lg border p-2.5 text-xs font-medium transition"
                  :class="userSkills.includes(tag.toLowerCase()) 
                    ? 'border-emerald-100 bg-emerald-50/50 text-emerald-800' 
                    : 'border-slate-150 bg-slate-50 text-slate-600'"
                >
                  <CheckCircle2 
                    class="h-4 w-4 shrink-0" 
                    :class="userSkills.includes(tag.toLowerCase()) ? 'text-emerald-600' : 'text-slate-300'" 
                  />
                  <div class="min-w-0 flex-1">
                    <p class="font-bold truncate text-[11px]">{{ tag }}</p>
                    <p class="text-[9px] text-slate-500 leading-none mt-0.5">
                      {{ userSkills.includes(tag.toLowerCase()) ? 'Perfil verificado ✅' : 'Habilidad deseable' }}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div class="space-y-2 rounded-lg border border-slate-150 bg-white p-4 shadow-sm">
              <h5 class="text-[11px] font-bold uppercase tracking-wider text-slate-400">Documentos verificados que se enviarán</h5>
              <div class="space-y-3 mt-1.5">
                <div class="flex items-start gap-2.5">
                  <FileText class="h-4.5 w-4.5 text-[#0B3A78] shrink-0 mt-0.5" />
                  <div class="text-xs leading-snug">
                    <p class="font-bold text-slate-800">Currículum Vitae Inteligente</p>
                    <p class="text-slate-500 text-[10px]">Tu CV unificado con historial laboral verificado en Tukuy Obra y SIADEG.</p>
                  </div>
                </div>
                <div class="flex items-start gap-2.5">
                  <Award class="h-4.5 w-4.5 text-emerald-600 shrink-0 mt-0.5" />
                  <div class="text-xs leading-snug">
                    <p class="font-bold text-slate-800">Certificados de Tukuy Academy ({{ portal.completedCourses.value.length }} completados)</p>
                    <p class="text-slate-500 text-[10px]">Tus acreditaciones aprobadas que validan tus capacidades técnicas.</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="space-y-1.5">
              <label class="text-[11px] font-bold uppercase tracking-wider text-slate-400" for="cover-letter">
              ...
              <textarea 
                ...
                class="w-full rounded-lg border border-slate-250 bg-white p-2.5 text-xs leading-relaxed focus:border-[#0B3A78] focus:ring-1 focus:ring-[#0B3A78] outline-none transition"
              />
            </div>
          </div>
          ...
          <div class="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50 px-6 py-4">
            <Button variant="outline" size="sm" @click="selectedJob = null">Cancelar</Button>
            <Button 
              size="sm" 
        ...
        <template v-else-if="applicationStep === 'submitting'">
          <div class="flex flex-col items-center justify-center p-10 text-center space-y-6">
            <Loader2 class="h-10 w-10 animate-spin text-[#0B3A78]" />
            <div class="space-y-2">
              <h4 class="text-sm font-bold text-slate-800">Procesando postulación en la Bolsa Tukuy</h4>
              <p class="text-xs text-slate-500 max-w-xs leading-relaxed">
                Nuestra Inteligencia de Match está cruzando tus credenciales con los requisitos de la constructora.
              </p>
            </div>
            
            <!-- Progress indicator steps -->
            <div class="w-full max-w-xs space-y-2 text-left text-xs bg-slate-50 p-4 rounded-lg border">
              <div class="flex items-center justify-between gap-3 text-slate-700">
                <span :class="submittingStep >= 0 ? 'font-bold text-[#0B3A78]' : 'text-slate-400'">
                  1. Cruzando perfil con requerimientos
                </span>
                <span v-if="submittingStep > 0" class="text-emerald-600 font-bold">✅</span>
                <Loader2 v-else-if="submittingStep === 0" class="h-3 w-3 animate-spin text-[#0B3A78]" />
              </div>
              <div class="flex items-center justify-between gap-3 text-slate-700">
                <span :class="submittingStep >= 1 ? 'font-bold text-[#0B3A78]' : 'text-slate-400'">
                  2. Adjuntando CV Inteligente y Certificados
                </span>
                <span v-if="submittingStep > 1" class="text-emerald-600 font-bold">✅</span>
                <Loader2 v-else-if="submittingStep === 1" class="h-3 w-3 animate-spin text-[#0B3A78]" />
              </div>
              <div class="flex items-center justify-between gap-3 text-slate-700">
                <span :class="submittingStep >= 2 ? 'font-bold text-[#0B3A78]' : 'text-slate-400'">
                  3. Registrando postulación oficial
                </span>
                <Loader2 v-if="submittingStep === 2" class="h-3 w-3 animate-spin text-[#0B3A78]" />
              </div>
            </div>
          </div>
        </template>
        ...
        <template v-else-if="applicationStep === 'success'">
          <div class="flex flex-col items-center justify-center p-10 text-center space-y-5">
            <div class="rounded-full bg-emerald-50 p-3.5 ring-8 ring-emerald-500/10">
              <CheckCircle2 class="h-8 w-8 text-emerald-600" />
            </div>
            <div class="space-y-2">
              <h4 class="text-base font-black text-slate-900">¡Postulación Enviada con Éxito!</h4>
              <p class="text-xs text-slate-500 max-w-xs mx-auto leading-relaxed">
                Tus certificados oficiales y tu currículum inteligente verificado han sido entregados a <strong>{{ selectedJob.company }}</strong>.
              </p>
            </div>
            <div class="rounded-lg bg-emerald-50/50 border border-emerald-100 p-3.5 text-xs text-emerald-800 max-w-sm text-left">
              💡 <strong>¿Qué sigue?</strong> La constructora revisará tu perfil directamente desde su panel de control de Tukuy Obra. Te notificaremos si eres seleccionado para entrevista.
            </div>
            <Button 
              size="sm" 
              class="bg-slate-900 text-white hover:bg-slate-800 font-semibold px-6 mt-2"
              @click="selectedJob = null"
            >
              Entendido
            </Button>
