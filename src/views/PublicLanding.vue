...
const benefits = [
  {
    ...
    description: 'Genera un CV laboral con cursos, experiencia y evidencias de obra.',
    icon: FileText,
  ...
  },
  {
    ...
    description: 'Recibe oportunidades compatibles con tu perfil técnico y disponibilidad.',
  ...
  },
  {
    ...
    description: 'Integra cargos, obras y módulos usados en Tukuy Obra o SIADEG.',
    icon: Link2,
    label: 'Tukuy Obra',
  ...
  },
  {
    ...
    description: 'Aprende según tu cargo, módulos de obra y objetivos laborales.',
    icon: Sparkles,
    label: 'Aprendizaje',
      ...
      <div class="grid gap-4 lg:grid-cols-3">
        ...
        >
          <div class="relative aspect-[16/9] overflow-hidden">
            ...
            <Badge class="absolute left-4 top-4 w-fit border-white/20 bg-white/90 text-slate-950" variant="outline">Operativo</Badge>
          </div>
          <CardHeader>
            <CardTitle class="text-xl">Almacén, pedidos y requerimientos</CardTitle>
        ...
        >
          <div class="relative aspect-[16/9] overflow-hidden">
            ...
            />
            <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-950/15 to-transparent" />
            <Badge class="absolute left-4 top-4 w-fit border-white/20 bg-white/90 text-slate-950" variant="outline">Técnico</Badge>
          </div>
          <CardHeader>
            <CardTitle class="text-xl">Avance, valorizaciones y reportes</CardTitle>
        ...
        >
          <div class="relative aspect-[16/9] overflow-hidden">
            ...
            />
            <div class="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-950/15 to-transparent" />
            <Badge class="absolute left-4 top-4 w-fit border-white/20 bg-white/90 text-slate-950" variant="outline">Inteligente</Badge>
          </div>
          <CardHeader>
    ...
    <section id="beneficios" class="mx-auto grid max-w-7xl gap-5 px-5 pb-14">
      <div class="grid gap-3 md:grid-cols-[0.9fr_1fr] md:items-end">
        <div>
          ...
          <h2 class="mt-2 max-w-3xl text-3xl font-black tracking-normal">Todo lo necesario para aprender y postular mejor</h2>
        </div>
        <p class="text-sm leading-6 text-white/68">
          Academy conecta capacitación, CV, certificados y oportunidades laborales en una sola ruta.
        </p>
      </div>
      ...
      <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        >
          <CardContent class="grid min-h-[190px] content-between gap-5 p-5">
            <div class="grid gap-4">
              <div :class="['flex h-11 w-11 items-center justify-center rounded-xl ring-1 ring-white/10', benefit.class]">
                <component :is="benefit.icon" class="h-5 w-5" />
              </div>
              <div class="grid gap-2">
                <Badge class="w-fit border-white/20 bg-white/90 text-slate-950" variant="outline">
                  {{ benefit.label }}
                </Badge>
                <h3 class="text-lg font-bold leading-tight">{{ benefit.title }}</h3>
                <p class="text-sm leading-6 text-white/65">{{ benefit.description }}</p>
              </div>
            </div>
          </CardContent>
        </Card>
