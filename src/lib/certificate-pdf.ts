...
export async function downloadPortfolioPdf(courses: Course[], user: UserProfile) {
  ...
  doc.setFontSize(11)
  doc.setTextColor(slate.r, slate.g, slate.b)
  doc.text(`Identificación: ${user.name.toLowerCase().replace(/\s+/g, '.')}@tukuyobra.com`, centerX, 90, { align: 'center' })
  doc.text(`Fecha de generación: ${new Date().toLocaleDateString('es-PE')}`, centerX, 96, { align: 'center' })
  ...
  const totalHours = courses.reduce((sum, c) => sum + Number.parseInt(c.duration, 10), 0)
  doc.text(`• Total de horas de capacitación técnica: ${totalHours} horas`, 30, 132)
  doc.text(`• Nivel de perfil alcanzado: ${user.profileProgress ?? 80}% en el CV inteligente`, 30, 138)
  doc.text(`• Código de autenticación global: TA-PORT-${user.name.substring(0, 3).toUpperCase()}`, 30, 144)
  ...
  courses.forEach((c, idx) => {
    ...
    doc.text(`Categoría: ${c.category}  |  Duración: ${c.duration}  |  Código: ${certCode}`, 20, currentY + 5)
  ...
  doc.text('Este portafolio consolida de forma digital las competencias verificadas del estudiante dentro de Tukuy Academy.', centerX, pageHeight - 15, { align: 'center' })
  ...
  for (const c of courses) {
    ...
    doc.setFontSize(11)
    doc.setTextColor(51, 65, 85)
    doc.text('ha completado satisfactoriamente el curso', cX, 96, { align: 'center' })
    ...
    doc.setFontSize(9)
    doc.setTextColor(100, 116, 139)
    doc.text(`Emitido el ${certData.issuedAt} · Cusco, Perú`, cX, detailsY + 24, { align: 'center' })
    ...
    doc.setTextColor(100, 116, 139)
    doc.text('Constancia verificable', 52, footerTop + 21)
    doc.text('Plataforma Tukuy Academy & Jobs', pageW - 52, footerTop + 21, { align: 'right' })
    ...
    doc.setFontSize(8)
    doc.setTextColor(teal.r, teal.g, teal.b)
    doc.text(`Código: ${certData.certificateCode}`, cX, footerTop + 15, { align: 'center' })
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(100, 116, 139)
    doc.text('Verificación: academy.tukuyobra.com/verificar', cX, footerTop + 21, { align: 'center' })
  }

  doc.save(`portafolio-certificados-${user.name.toLowerCase().replace(/\s+/g, '-')}.pdf`)
}
